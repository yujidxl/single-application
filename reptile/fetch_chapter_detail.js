...
const getChapterDetail = async function () {
  ...
  const chapter_list = await ChapterList.findAll({
    raw: true,
    where: { chapter_ready_pass: false },
    ...
    sub_queue.add(async () => {
      ...
      try {
        const res = await request
          .get(
            `${baseUrl}${chapter.chapter_id}/${chapter.chapter_detail_id}.html`
          )
        ...
        const chapter_detail = $('#htmlContent')
          .html()
          .replace(/(.+<br><br>)|(--&gt;&gt;.+)/g, '');
        chapter_obj = {
          chapter_detail,
        ...
        await ChapterList.update(
          { chapter_ready_pass: true },
          {
            where: {
              id: chapter.id,
            },
